 ...
 *
 * Measures the four things that determine how long a quote takes and how good it is:
 *
 ...
 *   4. local routing vs a hosted `/v2/rate` round trip, when a server is configured.
 *
 * Latency against live venues is not a controlled measurement: it moves with network conditions,
 ...
 *
 * Credentials are read from the environment (SOROSWAP_API_KEY, STELLARBROKER_PARTNER_KEY,
 * USWAP_API_URL, USWAP_API_KEY). Every provider runs without them except Soroswap, which is
 * skipped with a note rather than silently reported as a failure.
 */
...
const local = new StellarSwapSDK({ routing: 'local', credentials })
const server =
  process.env.USWAP_API_URL && process.env.USWAP_API_KEY
    ? new StellarSwapSDK({ routing: 'server', apiBaseUrl: process.env.USWAP_API_URL, apiKey: process.env.USWAP_API_KEY })
    : null
...
for (let i = 0; i < ITERATIONS; i++) {
  ...
  const q = await local.quote(PAIR)
...
console.log(`waterfall picks: ${Object.entries(picks).map(([p, n]) => `${p} ${n}/${ITERATIONS}`).join(', ')}`)
console.log()
...
console.log(`waterfall over 4 routes : ${solverUs.toFixed(2)} µs/call  (${SOLVER_RUNS.toLocaleString()} runs)`)
...
// ---------------------------------------------------------------------------
// 4. Local routing vs a hosted /v2/rate round trip.
// ---------------------------------------------------------------------------
...
if (server) {
  for (let i = 0; i < ITERATIONS; i++) {
    const started = Date.now()
    await server.quote(PAIR).catch(() => null)
    serverWall.push(Date.now() - started)
  }
  console.log('## 4. Local fan-out vs hosted /v2/rate (ms)')
  console.log(HEADER)
  console.log(row('local', stats(fanoutWall)))
  console.log(row('hosted /v2/rate', stats(serverWall)))
  console.log()
} else {
  console.log('## 4. Local vs hosted — skipped (set USWAP_API_URL and USWAP_API_KEY to include it)\n')
}
...
if (JSON_OUT) {
  const payload = {
    ...
    solverUs,
    discoveryUs,
    server: server ? stats(serverWall) : null
  }
